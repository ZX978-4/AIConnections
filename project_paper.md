...
### 2.1 模型上下游查询的核心技术
- **关系数据建模与存储**：采用节点（模型/数据集）和边（依赖/衍生）结构，利用CSV文件高效存储，便于批量处理和快速加载。
- **高效关系遍历算法**：基于广度优先搜索（BFS）实现模型的上游（祖先）和下游（后代）关系查询，支持自定义深度，避免环路和重复节点，保证查询的准确性和性能。
- **RESTful API 设计**：通过Flask框架提供统一的接口，支持模型检索、关系查询、血缘谱系构建等功能，便于前端灵活调用。

### 2.2 可视化呈现的核心技术
- **D3.js 动态关系图绘制**：利用D3.js对后端返回的血缘数据进行SVG可视化，支持节点分层布局、连线、箭头、缩放、拖拽等交互。
- **分层布局与自适应排布**：根据节点深度自动分组，中心节点居中，上下游节点分布两侧，动态计算坐标，避免重叠，提升可读性。
- **高交互性UI设计**：实现模型搜索、节点点击切换、悬浮提示、统计信息展示等功能，提升用户体验。

### 2.3 完整性验证模块的核心技术
- **哈希校验算法**：对模型及其血缘关系数据进行哈希计算，生成唯一性标识，支持模型内容和结构的完整性验证。
- **血缘完整性检查**：自动遍历模型的上下游关系，统计依赖数量、关系链路，检测血缘链条的完整性和一致性。
- **标准化SBOM导出**：依据SPDX标准自动生成模型及其依赖的物料清单（SBOM），便于合规审计和第三方验证。

上述技术方案协同实现了AI模型供应链的高效查询、可视化分析与可信验证，为平台的安全性和可用性提供了坚实基础。
